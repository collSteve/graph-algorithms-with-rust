...
pub trait Graph<TVertex: Vertex<TData>, TEdge: Edge, TData> {
    ...
    fn get_edge(&self, id: VertexId) -> Result<&TEdge, GraphError> {
        let edge_option = self
            .get_all_edges()
            .iter()
            .find(|&edge| edge.get_id() == id);
        match edge_option {
...
impl<TData, TVertex: Vertex<TData>, TEdge: Edge> Graph<TVertex, TEdge, TData>
    for BaseGraph<TVertex, TEdge>
{
    fn get_vertex_count(&self) -> i32 {
